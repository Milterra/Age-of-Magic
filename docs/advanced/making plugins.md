...
- `BuildCtx` is defined in `quartz/ctx.ts`. It consists of
  - `argv`: The command line arguments passed to the Quartz [build](node_modules/mathjax-full/components/bin/build) command
  - `cfg`: The full Quartz [configuration](docs/configuration.md)
  - `allSlugs`: a list of all the valid content slugs (see [paths](docs/advanced/paths.md) for more information on what a `ServerSlug` is)
...
A good example of a transformer plugin that borrows from the `remark` and `rehype` ecosystems is the [Latex](docs/plugins/Latex.md) plugin:
...
- Your component should use `getQuartzComponents` to declare a list of `QuartzComponents` that it uses to construct the page. See the page on [creating components](docs/advanced/creating%20components.md) for more information.
