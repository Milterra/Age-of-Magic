...
4. **A new extensible [configuration](docs/configuration.md) and [](docs/configuration.md#Plugins|plugin) system**: Quartz 3 was hard to configure without technical knowledge of how Hugo's partials worked. Extensions were even hard to make. Quartz 4's configuration and plugin system is designed to be extended by users while making updating to new versions of Quartz easy.

## Things to update

- You will need to update your deploy scripts. See the [hosting](docs/hosting.md) guide for more details.
- Ensure that your default branch on GitHub is updated from `hugo` to `v4`.
- [Folder and tag listings](docs/features/folder%20and%20tag%20listings.md) have also changed.
...
- If you customized the layout of Quartz 3, you may need to translate these changes from Go templates back to JSX as Quartz 4 no longer uses Hugo. For components, check out the guide on [creating components](docs/advanced/creating%20components.md) for more details on this.
