...
By default, Quartz will title the page `Folder: <folder name>` and no description. You can override this by creating an `index.md` file in the folder with the `title` [](docs/authoring%20content.md#Syntax|frontmatter) field. Any content you write in this file will also be used in the folder description.
...
You can link to the tag listing by referencing its name with a `tag/` prefix, like this: `[plugin](docs/tags/plugin.md)` (results in [plugin](docs/tags/plugin.md)).
...
Quartz allows you to define a custom sort ordering for content on both page types. The folder listings are a functionality of the [FolderPage](docs/plugins/FolderPage.md) plugin, the tag listings of the [TagPage](docs/plugins/TagPage.md) plugin. See the plugin pages for customization options.
